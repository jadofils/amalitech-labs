...
- `<Class>Test.java` - behavioural tests using real collaborators (or, for the
  main.service/main.manager layers, a hand-wired real dependency chain).
- `<Class>MockitoTest.java` - the same class verified through mocked
  ...
  main.repository returning `null`) that the real implementation can never
...
The required libraries (`junit-platform-main.console-standalone`, `mockito-core`,
...
java -jar junit-platform-main.console-standalone-*.jar execute \
     -cp "out/main;out/test;<jars>" --scan-classpath --include-classname=".*Test"
```

where `<jars>` is `junit-platform-main.console-standalone`, `mockito-core`,
